...
`v0.85.0` 起，`prompt_asset_id` 不再只是任务包里的字符串。新增文件型 Prompt Registry：`templates/prompt_assets/*.md` 注册路由级提示词资产，`prompt-registry-validate` 会检查资产字段并确认 task registry 中全部 prompt id 都能解析，`prompt-preview` 可预览任一任务 ID 的实际提示词资产，`task-open` 生成的任务包会自动附带解析后的 Prompt Asset、输出契约和正文提示。
...
`v0.90.1` 起，前端收束为三块：项目总控、文风挂载、模型配置。旧创作总监聊天和大段文风工坊表单从前端移除，避免把平台 Agent 的职责误解成本地网页职责；总控面板改为中文、人话化展示，不再直接暴露原始 JSON，而是把 route gate、sidecar、next action 和事件记录包装成项目进度驾驶舱。

`v0.90.2` 起，前端进一步明确“JSON 信息不是隐藏，而是包装后展示”。总控页新增项目证据柜，把 `workflow-dashboard` 里的状态机摘要、平台任务摘要、路线审计、输出路径、规则边界和最近事件拆成中文证据卡。模型配置不再作为一级创作模块出现，而是降级为总控页里的“连接设置”：保留 API Key、API 地址、模型名和默认目录配置能力，主要服务本地 API、Dify/LangGraph 或非 Codex/Claude 宿主测试；正式创作仍由平台 Agent 通过 CLI 状态机执行。
...
- New Character Register：已完成场景中新角色声明、候选资产/approval 门禁、promotion/readiness/route-audit 接入。
