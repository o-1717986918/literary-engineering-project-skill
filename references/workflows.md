...
Generated candidates and promoted drafts should also pass the AI trace reduction gate. `review-scene` reports mechanical “不是……而是……” frames, dash variants such as “不是……——是……”, abstract summary language, explanatory psychology labels, template transitions, symmetric slogan rhythm, omniscient theme explanation, and aphoristic endings under `AI Trace Reduction Test`. Treat these as semantic revision signals unless the project records a deliberate stylistic exception. Do not run regex cleanup that deletes negation or rewrites contrast frames; the platform agent must decide sentence by sentence whether a frame is functional rhetoric or model habit.
