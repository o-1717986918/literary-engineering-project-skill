...
For longform projects, `chapter_id` must map to `plot/word_budget/word_budget.json` and to a completed `plot/chapter_obligations/{chapter_id}.json` reader contract. `word_count_target`, `word_count_min`, and `word_count_max` are optional scene-level overrides, but when present they are hard generation and review properties measured as Chinese-content characters. Context packets, composition packets, prompt manifests, generation sidecars, AgentReview, promotion gates, route audits, chapter readiness, and formal exports should use cleaned deliverable prose when checking these values.
...
## Context Packets And Context Trace
...
- `*.md`: compact working memory, hard canon summary, character state, style constraints, word-budget contract, and soft retrieval results;
- `*.trace.json`: machine-readable source proof for the context packet.

Soft retrieval never overrides canon.

`context_trace.v1` records:

- route and scene id;
- context packet path and scene file path;
- required context groups;
- loaded files;
- summarized retrieval files;
- excluded files, especially non-loaded secondary/cameo character files;
- mounted style sources;
- word-budget source;
- loaded character files and canon files;
- token/length budget;
- missing required context.

Formal scene-development treats a context packet without its trace as incomplete. `task-next`, `task-complete`, `workflow-state`, `route-audit`, `chapter-workspace`, `generate-scene`, `compose-scene`, and AgentReview should all inspect or preserve the trace path. If trace is missing, invalid, points to the wrong scene/context packet, or reports missing required context, rerun/fix `context` before roleplay, branch, generation, review, promotion, chapter readiness, or export.
...
Per-chapter reader contracts are generated by `chapter-obligation`:
...
For longform scene generation, the current scene must map to a completed per-chapter contract through `chapter_id` or `chapter_obligation_id`. `reader_experience_by_scene` is a generation/review contract, not optional notes.
...
Generated or maintained by maintainers:
...
Prompt assets use frontmatter plus Markdown body. The frontmatter records `prompt_asset_id`, optional wildcard `match`, version, route, task_type, required_inputs, context_groups, hard_constraints, output_contract, review_requirements, and forbidden_shortcuts. The body is the actual platform-Agent-facing route prompt.

`task-open` resolves a task's `prompt_asset_id` through this registry and injects the matched Prompt Asset into `workflow/tasks/{task_id}.agent_tasks.md`. If a route adds or renames prompt ids, run `prompt-registry-validate` and keep every task id covered by an exact or wildcard asset.
...
These files are diagnostic dashboards and formal route ledgers. They summarize `.agent_tasks.md` sidecars, expected artifact paths, missing outputs, inferred routes, route-specific gates, promotion review gates, unresolved scene review notes when auditing `scene-development`, workflow event/task integrity, and cross-route next actions. `workflow-dashboard` additionally writes `workflow/dashboard/workflow_dashboard.json`, `.md`, and `.html` as a read-only cockpit for platform agents or local frontends. They do not create prose, canon, characters, final plot, approval, or workflow completion. A pending sidecar remains pending until the platform agent reads it, writes the expected artifacts, checks them, and creates the adjacent `.agent_completion.json` marker.

Generated by `workflow-state`:
...
Agent mode must also include a platform-agent execution gate and reading receipt. The platform agent should list the scene, context packet, context trace, character files, canon files, missing sources, hard constraints, and writeback boundary before filling character proposals, world consequences, branch candidates, Director scoring, Canon Auditor notes, or writeback candidates.
...
`revise-scene` is the formal notes-resolution loop for a reviewed scene. It reads the scene file, draft body, context packet, context trace, AgentReview JSON or static review, mounted style, canon, punctuation standard, and word budget. The main platform agent then writes a revised candidate and a report that lists which review notes were applied, what still needs approval, and why the result is ready for re-review. Subagents may list issues or evidence, but must not write the revised body text.

The revision report must include anti-evasion evidence when a contrast, transition, dash, AI-trace phrase, or Style Lint item is touched. Required rows: original sentence, original issue, revised sentence, whether an explicit transition remains, disguised-replacement risk, retained-transition reason, critical counterargument, and conclusion. Weak reasons such as “增强节奏”“体现复杂心理” or “更有文学感” are not enough. The manifest should record `anti_evasion_protocol_applied=true`, `anti_evasion_rows`, `retained_transition_proofs`, and `evasion_risks_unresolved`; unresolved evasion risks block formal route audit.
...
- Style Lint Gate must be clean or notes-only: `mechanical-contrast-frame`, `contrast-evasion-frame`, and medium-or-higher AI trace findings block promotion/readiness/export even if AgentReview JSON says `pass`;
