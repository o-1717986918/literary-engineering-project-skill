...
8. 后续 `generate-scene` 的 prompt manifest 自动加载 `plot/word_budget/word_budget.json` 中的预算标准。
9. `route-audit --route longform-planning` 和 `longform-audit` 检查预算是否存在、是否 needs_expansion、场景库存是否明显不足。
...
- 当前章是否存在正文字数欠账或缺失场景，且这些欠账是否已有扩场景任务。
- prompt manifest 是否包含“长篇字数预算标准”。

如果预算为 `needs_expansion`，或 `route-audit --route longform-planning` 显示场景库存门禁未完成，不得直接批量生成正文；先扩充大纲和场景库存。
