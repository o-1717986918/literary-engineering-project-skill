...
- When a Style Skill is mounted, formal platform scene review must include `style_adherence` in `reviews/agent/{scene_id}_scene_review.json`. `style_adherence.status` must be `pass` or `pass_with_notes` before promotion, chapter readiness, export, or writeback; `not_applicable`, missing fields, or `revise_required` are blocking until `revise-scene` or an explicit waiver resolves them.
