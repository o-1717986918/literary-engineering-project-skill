...
For formal scene work, longform planning, and source ingestion, treat `task-next`, `task-open`, `task-submit`, and `task-complete` as the controlling loop when available. The platform agent still performs the creative judgment and writes prose/reviews/branch choices/budgeted outline candidates/scene inventory candidates/source-derived candidate files, but the CLI task registry chooses the next formal operation, provides the execution package, records submissions, and blocks missing expected outputs.
