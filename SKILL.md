...
- Longform word budgets are generation constraints, not final plot. For 100000+ word or multi-volume targets, build or inspect `plot/word_budget/word_budget.json`, have the platform agent handle the budget expansion sidecar, and do not bulk-generate while budget status is `needs_expansion`.
...
- `pass_with_notes` is not a silent pass. The writing agent must apply listed `revision_actions` / warnings / style notes as local edits, or record a specific acceptance/waiver reason before promotion, chapter readiness, export, or writeback.
...
- Longform planning: read `docs/modules/longform-word-budget.md`, run or emulate `word-budget`, process the platform-agent expansion sidecar, and keep the budgeted outline as a candidate until review and approval.
