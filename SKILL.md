...
- Generated prose should reduce visible AI habits: mechanical “不是……而是……” contrast frames and dash variants such as “不是……——是……” are prohibited by default unless the user or mounted Style Skill explicitly authorizes them as functional rhetoric. Also avoid abstract summary language, explanatory psychology labels, template transitions, symmetric slogan rhythm, omniscient theme explanation, and aphoristic endings unless a scene intentionally needs them. Scripts may flag or safely normalize typography, but must not batch-delete negation or perform semantic prose cleanup.
