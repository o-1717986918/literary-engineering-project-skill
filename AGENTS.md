...
11. `docs/modules/workflow-state-machine.md` when changing task state, event logs, route validation, sidecar completion, or workflow dashboards
12. `references/orchestration.md` only for LangGraph, Dify, subagents, or external workflow design
...
11. Use `agent-task-status`, `workflow-state`, `workflow-validate`, or `route-audit` whenever pending sidecars, expected artifacts, state/event integrity, or route gates are unclear.
12. For formal `scene-development`, `longform-planning`, `source-ingest`, `style-engineering`, `character-and-world-assets`, `review-and-audit`, and `export-and-release`, use `task-next`, `task-open`, `task-submit`, and `task-complete` as the controlling loop when available; do not choose the next formal step from memory.
...
- Longform word-budget planning is also platform-agent work: the CLI may calculate target distribution and inventory gaps, but the platform agent expands the outline, judges pacing/load, creates chapter-obligation/reader-experience contracts, and decides readiness. For formal work, run `task-next --route longform-planning` until it returns ready; budget, scene-inventory, and chapter-obligation sidecars, candidates/reviews, completion markers, and clean `pass` reviews are required.
- Character/world asset creation is also platform-agent work: `asset-create` / `agent-create-*` only prepares a sidecar. The platform agent writes candidate JSON/report, then runs `review-candidate-asset`, performs the semantic review, asks for user approval, and only then promotes with an approval run id. For formal work, run `task-next --route character-and-world-assets` until it returns ready.
- Project review and release audit are also platform-agent work: `canon-lint` and `longform-audit` provide deterministic evidence, but the platform agent must write the canon review and committee review artifacts. For formal work, run `task-next --route review-and-audit` until canon review is a clean pass, longform audit exists, and the committee returns approve with no action items.
...
- `agent-task-status`, `workflow-state`, `workflow-validate`, `route-audit`, and `workflow-dashboard` are dashboard helpers. They do not complete creative work; they reveal unhandled sidecars, missing expected artifacts, state/event/task inconsistencies, incomplete route gates, and cross-route next actions for the platform agent to resolve or list as pending.
...
- Workflow Contract Validation is the v0.87.0 state/event/task integrity gate. Run `workflow-validate` when route state looks inconsistent, before trusting bulk automation, and before release audits that depend on a long task ledger.
- Reader Experience Contract is the v0.88.0 longform quality gate. For 100000+ Chinese-content-character or multi-volume projects, run `chapter-obligation --chapter-id <chapter_id>` before prose for that chapter, let the platform agent fill the chapter/scene contract, and treat missing `reader_experience_adherence` as blocking in review/promotion/export.
- Workflow Dashboard is the v0.89.0 read-only cockpit. Run `workflow-dashboard <project>` to refresh overall route state, sidecar status, route audits, recent events, and static HTML/JSON/Markdown dashboard files without advancing the project or bypassing `task-next` / `task-complete`.
- Style engineering is platform-agent work: metrics and profiles are evidence, but the platform agent must write the LLM-facing `style_prompt.md`, satisfy the 500-2500 Chinese-content-character quality contract, complete the style prompt sidecar, and obtain at least one accepted style evaluation before formal build/mount.
...
- For 100000+ Chinese-content-character or multi-volume targets, create or inspect `plot/word_budget/word_budget.json` before bulk generation. If budget status is `needs_expansion`, process `word_budget.agent_tasks.md`, process `scene_inventory_expansion.agent_tasks.md`, process `plot/chapter_obligations/chapter_obligations.agent_tasks.md`, and review the budgeted outline, scene inventory, and chapter-obligation candidates first. Each formal scene needs a budgetable `chapter_id`; `word_count_target`, `word_count_min`, and `word_count_max` in `scene.yaml` become generation/review constraints, and only cleaned deliverable prose counts.
- For longform scene generation, current chapter must have a completed `plot/chapter_obligations/{chapter_id}.json` contract and sidecar marker. The contract must map each scene to reader_question, promised_reward, withheld_information, payoff_or_delay, tension_source, anti_summary_requirement, and reader_aftertaste. Do not write formal prose while this gate is blocked.
- User-facing target length means cleaned Chinese-content characters, including Han characters and Chinese punctuation. Machine non-whitespace counts are diagnostics only and must not be used as the formal pass/fail basis when they disagree with Chinese-content counts. Use `machine_count_mapping` only to explain UI/platform count differences, not to pass an underbuilt scene.
...
- Mountable Style Skills require a reliable LLM-facing `prompt.md` with 500-2500 Chinese-content characters, counting Han characters and Chinese punctuation after Markdown scaffolding is stripped, plus complete high-quality prompt blocks: identity/boundary, priority, core mechanism, narrative distance, syntax/rhythm, punctuation, imagery/sensory, psychology/behavior, dialogue/tone, forbidden tendencies, and output self-check.
