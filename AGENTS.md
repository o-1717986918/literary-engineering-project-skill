...
- Mounted Style Skills require a formal `style_adherence` review in `reviews/agent/{scene_id}_scene_review.json`. When a style is mounted, `style_adherence.status` must be `pass` or `pass_with_notes` before promotion, chapter readiness, export, or state writeback; missing, `not_applicable`, or `revise_required` is blocking until revised or explicitly waived.
