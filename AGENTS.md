...
12. For formal `scene-development`, `longform-planning`, `source-ingest`, `style-engineering`, and `character-and-world-assets`, use `task-next`, `task-open`, `task-submit`, and `task-complete` as the controlling loop when available; do not choose the next formal step from memory.
