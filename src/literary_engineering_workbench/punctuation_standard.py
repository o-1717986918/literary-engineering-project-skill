...
- 破折号只用于真正的插入、打断、骤然转向、话语中断或强烈解释性补充。好的破折号承担节奏、讽刺顿挫、信息纠偏或话语中断；坏的破折号只是把“但是/而是/于是”换成装饰性停顿。不要把“——”当成通用转折、强调或高级感装饰；同一段内反复破折号会削弱节奏。
- 未获 Style Skill 或用户明确授权时，同一自然段破折号原则上不超过 2 处，全场景破折号总数原则上不超过 12 处。获授权的高破折号文风可以突破数量，但平台 agent 必须逐句判断每处破折号是否承担叙事功能，不得按正则批量删改。
...
PUNCTUATION_STANDARD_SHORT_RULE = (
    ...
    "同时控制文学节奏：句号用于真实语义落点，逗号承接未完成关系，破折号只用于打断/插入/骤变/功能性纠偏，"
    "未获文风授权时不要让破折号替代“而是”或通用转折；转折优先由动作、意象和因果生成，避免机械堆叠“但是、然而、于是、然后”。"
    ...
    if dash_count >= 4 or any(paragraph.count("——") >= 3 for paragraph in prose.splitlines() if paragraph.strip()):
        issues.append(
                PunctuationIssue(
                    "dash-overuse",
                    "medium",
                    "破折号使用过密。请区分功能性破折号和机械装饰：前者承担插入、打断、骤然转向、信息纠偏或讽刺顿挫，后者只是替代“而是/但是/于是”。普通转折、强调和伪文学停顿应改用句法、逗号、分号或动作承接；不得用脚本批量删除导致语义反转。",
